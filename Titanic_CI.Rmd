...
conf_Neuralnet <- confusionMatrix(prediction$net.result, dataValfe$Survived)

# Random Forests
set.seed(415)
fit_rf<- randomForest(Survived~Pclass+Sex+CoW+FamilyCat+Title, data=dataTrainfe,type=classification, ntree=100,mtry=2)

prediction$Pred_rffe <- predict(fit_rf,type="prob",dataValfe) [,2]
...
#SVM modelling
plot(dataTrainfe$Age, dataTrainfe$Pclass, xlab="Age", ylab="Pclass", col=ifelse(dataTrainfe$Survived==1, "red", "blue"))
